...
function defaultCommand() {
  switch (os.platform()) {
    case "darwin":
      return 'open -a Terminal.app "$PWD"';
    case "win32":
      return 'start /D "%cd%" cmd';
    default:
...
function openTerminal(filepath, command) {
  ...
  const cmd = atom.config.get("terminal-spawn.command");
  return spawnCommand(cmd, dirpath);
}
...
module.exports = {
  config: {
    command: {
      ...
      description: "Shell command executed to launch the terminal.",
      type: "string",
